    ...
    if (!isset($_SESSION['user_id'])) {
        ...
        if (empty($respuesta_texto)) {
            ...
            try {
                ...
                header("Location: tema_detalle.php?id={$tema_id}&status=success_reply");
                exit;
...
if (isset($_GET['status']) && $_GET['status'] === 'success_reply') {
    $mensaje_status = '¡Tu respuesta ha sido publicada con éxito!';
...
<head>
    ...
    <style>
        :root {
            --primary: #ffffffff; /* Blanco (Usado principalmente para fondos de tarjetas/header) */
            --secondary: #4ecdc4; /* Teal (Acento secundario) */
            --dark: #2d3436; /* Gris Oscuro (Texto principal) */
            --light: #f7f1e3; /* Fondo del Cuerpo */
            --theme-green: #69A64A; /* Verde de la marca (Acento principal) */
        }
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                <form action="tema_detalle.php?id=<?php echo $tema_id; ?>" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="respuesta_texto" name="respuesta_texto" rows="4" required
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-1 focus:ring-accent focus:border-accent"
