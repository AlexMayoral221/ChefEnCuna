...
function add_simulated_category($receta) {
    ...
    $receta['categoria'] = $categories[$receta['id'] % count($categories)];
    return $receta;
}
...
<head>
    ...
    <style>
        ...
        .dropdown-menu a:hover {
            background-color: #588e43 !important;
        }
        .container {
            max-width: 1200px; 
            margin: 0 auto; 
        ...
        <nav class="app-nav flex items-center">
            ...
            <?php if(isset($_SESSION['user_id'])): ?>
                <div class="relative ml-4" x-data="{ open: false }" @click.away="open = false">
                    ...
                    <div x-show="open" x-transition class="absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 text-white ring-1 ring-black ring-opacity-5 dropdown-menu">
                        <a href="<?= htmlspecialchars($dashboard_url) ?>" class="block px-4 py-2 text-sm text-white">
                            <i class="fas fa-gauge-high mr-2"></i> Mi perfil
                        </a>
                        <a href="logout.php" class="block px-4 py-2 text-sm text-white border-t border-white/25">
            ...
            <?php else: ?>
                <div class="relative ml-4" x-data="{ open: false }" @click.away="open = false">
                    ...
                    <div x-show="open" x-transition class="absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 text-white ring-1 ring-black ring-opacity-5 dropdown-menu">
                        <a href="login.php" class="block px-4 py-2 text-sm text-white">
                            <i class="fas fa-sign-in-alt mr-2"></i> Entrar
                        </a>
                        <a href="registro.php" class="block px-4 py-2 text-sm text-white border-t border-white/25">
        ...
        <div class="grid" aria-live="polite">
            ...
            ?>
                <div class="card"
                    ...
                    <img src="<?= $imagePath ?>"
                         onerror="this.onerror=null;this.src='https://via.placeholder.com/400x220/69A64A/ffffff?text=<?= rawurlencode($categoriaReceta) ?>';"
                         class="card-img" 
                         alt="Receta de <?= $title ?>">
